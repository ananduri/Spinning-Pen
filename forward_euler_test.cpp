...
class ForwardEulerTest : public ::testing::Test {
protected:
...
TEST_F(ForwardEulerTest, IntegratingForZeroTimeShouldReturnSameState) {
  // SETUP
  ...
  // ACTION
  euler.integrate(pen.get_mutable_state(), dynamics, initial_time, duration);

  // VERIFICATION
  EXPECT_EQ(initial_state, pen.get_state());
}

TEST_F(ForwardEulerTest,
  ...
  // ACTION
  euler.integrate(pen_integrate.get_mutable_state(), dynamics, initial_time,
                  duration, step_size);
  euler.integrate(pen_step.get_mutable_state(), dynamics, initial_time,
                  duration, step_size);
...
TEST_F(ForwardEulerTest, IntegratingFallingPenShouldMatchAnalyticResult) {
  // SETUP
  const State initial_state{
      .linear_d = Vec3::Zero(),
      .linear_v = 1 * Vec3::UnitZ(),
